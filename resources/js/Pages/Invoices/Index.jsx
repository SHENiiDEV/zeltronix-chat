            ...
            header={
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-sm text-slate-400">View and download official B2B PDF invoices for token top-ups and service payments</p>
                    </div>
                </div>
            }
        >
            <Head title="Invoices & Billing History | Voltoria AI" />
                ...
                {invoices.length === 0 ? (
                    ...
                    <div className="divide-y divide-slate-800">
                        {invoices.map((inv) => {
                            ...
                            const refNumber = inv.gateway_reference || inv.invoice_number;
                            return (
                                <div key={inv.id} className="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-800/40 transition-colors">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-3">
                                                <span className="font-mono font-bold text-white text-base">{refNumber}</span>
                                            ...
                                            <p className="text-xs text-slate-400 mt-1">
                                                {inv.service_name || inv.description} • {new Date(inv.paid_at || inv.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <a
                                            ...
                                            className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white text-xs font-bold px-4 py-2.5 rounded-xl shadow-lg shadow-blue-500/20 transition-all flex items-center gap-1.5"
                                        >
                                        ...
                                        >
                                            View <ExternalLink className="w-4 h-4 text-blue-400" />
                                        </Link>
                                    </div>
                                </div>
                            );
